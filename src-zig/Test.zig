...
def.defrep.width.string(), def.defrep.scal.string(), def.defrep.long.string(),
def.defrep.hs,
sql3.boolean(def.defrep.hs),
sql3.cbool(def.defrep.hs)});

// Print("\r\n{s}\n",.{
// def.defrepList.items[0].name});
...
db.close();
    ...
    for (def.rows.items,0..) | _,n| {
        Print("\r\n{s}  {s}  {s}  {s}  {s} {s} {s}  {}  \n",.{
            def.rows.items[n].name.string(),def.rows.items[n].text.string(),
            def.rows.items[n].mnmo.string(),def.rows.items[n].type.string(),
            def.rows.items[n].width.string(),def.rows.items[n].scal.string(),def.rows.items[n].long.string(),
            def.rows.items[n].hs
        });
    }   
    Print("\r\nnbr rows {d} \n",.{def.rows.items.len});
