...
const defrepSQL = struct {
     refname: sql3.Text, text: sql3.Text, mnmo: sql3.Text, type: sql3.Text,
     width: sql3.Numeric, scal: sql3.Numeric, long: sql3.Numeric, hs: sql3.Bool };
...
pub fn createTable(db: sql3.Database) bool {
    ...
    db.exec(
        \\ CREATE TABLE "defrep" (
        \\ "refname" VARCHAR(25) NOT NULL UNIQUE,
    ...
    , .{}) catch |err| {
        ...
        @panic(std.fmt.allocPrint(allocREP,
         "\n\n\r file:{s} line:{d} column:{d} func:{s}() exec:CREATE TABLE defrep err:{}\n\r",
         .{ s.file, s.line, s.column, s.fn_name, err }) catch unreachable);
    };
    return true;
}

pub fn removeTable(db: sql3.Database) bool {
    // To work in extended digital (DCML) put the TEXT fields and BOOL
    if (!db.istable("defrep")) return false;
    db.exec("DROP TABLE defrep", .{}) catch |err| {
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP,
         "\n\n\r file:{s} line:{d} column:{d} func:{s}() exec:DROP TABLE defrep   err:{}\n\r",
         .{ s.file, s.line, s.column, s.fn_name, err }) catch unreachable);
    };
    return true;
}

pub fn existRows(db: sql3.Database, refname: []const u8) bool {
    const Result = struct { count: usize };

    var sqlwrk: []const u8 = undefined;

    sqlwrk = std.fmt.allocPrint(allocREP,
     "SELECT count(*) as count FROM defrep  WHERE refname='{s}'; ",
     .{refname}) catch unreachable;
    ...
    const select = db.prepare(
        ...
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP,
         "\n\n\r file:{s} line:{d} column:{d} func:{s}() refname:{s}  prepare:{s}   err:{}\n\r",
         .{ s.file, s.line, s.column, s.fn_name, refname, sqlwrk, err }) catch unreachable);
    };
...
pub fn insert(db: sql3.Database) bool {
    if (!db.istable("defrep")) return false;
    if (existRows(db, defrep.refname.string())) return false;
    const insertSQL = db.prepare(
        defrepSQL,
        void,
        \\INSERT INTO defrep (refname,text,mnmo,type,width,scal,long,hs)
        \\VALUES(:refname, :text, :mnmo, :type, :width, :scal, :long,:hs)
        ,
    ) catch |err| {
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP,
    ...
    insertSQL.exec(.{
        .refname = sql3.text(defrep.refname.string()),
...
pub fn update(db: sql3.Database) bool {
    ...
    , .{
        ...
        sql3.cbool(defrep.hs), defrep.refname.string(),
    }) catch |err| {
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP,
        "\n\n\r file:{s} line:{d} column:{d} func:{s}() err:{}\n\r",
...
pub fn delete(db: sql3.Database, refname: []const u8) bool {
    if (!db.istable("defrep")) return false;
    if (!existRows(db, defrep.refname.string())) return false;
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP, "DELETE FROM defrep  WHERE refname='{s}'", .{refname}) catch unreachable;
    defer allocREP.free(sqlwrk);

    db.exec(sqlwrk, .{}) catch |err| {
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP,
         "\n\n\r file:{s} line:{d} column:{d} func:{s}() refname:{s} exec:{s} err:{}\n\r",
         .{ s.file, s.line, s.column, s.fn_name, refname, sqlwrk, err }) catch unreachable);
    };
    if (!existRows(db, defrep.refname.string())) return true;
    return false;
}

// logique query
pub fn lgqQUERY(ldbr: sql3.Database, refname: []const u8, like: []const u8) void {
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP, "SELECT * FROM defrep WHERE {s} LIKE '%{s}%' ORDER BY refname ASC;",
    .{ refname, like }) catch unreachable;
    ...
    const select = ldbr.prepare(
        ...
        @panic(std.fmt.allocPrint(allocREP,
         "\n\n\r file:{s} line:{d} column:{d} func:{s}() refname:{s}  prepare:{s} err:{}\n\r",
         .{ s.file, s.line, s.column, s.fn_name, like, sqlwrk, err }) catch unreachable);
    };
    ...
    while (select.step() catch unreachable) |rcd| {
        defrep.refname.setZfld(rcd.refname.data);
        ...
        rows.append(allocREP, defrep) catch |err| {
            const s = @src();
            @panic(std.fmt.allocPrint(allocREP,
             "\n\n\r file:{s} line:{d} column:{d} func:{s}() refname:{s} select:{s} err:{}\n\r",
             .{ s.file, s.line, s.column, s.fn_name, like, sqlwrk, err }) catch unreachable);
        };
    }
}
...
pub fn pgDown(ldbr: sql3.Database, kpg: []const u8, nrows: u64) void {
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP,
     "SELECT * FROM defrep WHERE refname >= '{s}' ORDER BY refname ASC LIMIT {d}; ",
     .{kpg , nrows}) catch unreachable;
    ...
    const select = ldbr.prepare(
        ...
        @panic(std.fmt.allocPrint(allocREP,
         "\n\n\r file:{s} line:{d} column:{d} func:{s}() refname:{s} prepare:{s} err:{}\n\r",
         .{ s.file, s.line, s.column, s.fn_name, kpg, sqlwrk, err }) catch unreachable);
    };
...
// logique page Up
// n = nbr rows for grid
// position first ligne  
pub fn pgUp(ldbr: sql3.Database, kpg: []const u8, nrows: u64) void {
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP,
     "SELECT * FROM defrep WHERE refname <= '{s}' ORDER BY refname DESC LIMIT {d};",
     .{kpg , nrows }) catch unreachable;
    ...
    pgDown(ldbr, defrep.refname.string(), nrows);
