...
const defrepSQL = struct { name: sql3.Text, text: sql3.Text, mnmo: sql3.Text, type: sql3.Text, width: sql3.Numeric, scal: sql3.Numeric, long: sql3.Numeric, hs: sql3.Bool };
...
pub fn existRows(db: sql3.Database, name: []const u8) bool {
    ...
    sqlwrk = std.fmt.allocPrint(allocREP, "SELECT count(*) as count FROM defrep  WHERE name='{s}'; ", .{name}) catch unreachable;
    ...
    const select = db.prepare(
        ...
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP, "\n\n\r file:{s} line:{d} column:{d} func:{s}() name:{s}  prepare:{s}   err:{}\n\r", .{ s.file, s.line, s.column, s.fn_name, name, sqlwrk, err }) catch unreachable);
    };
...
pub fn update(db: sql3.Database) bool {
    ...
    , .{
        ...
        sql3.cbool(defrep.hs), defrep.name.string(),
    }) catch |err| {
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP, "\n\n\r file:{s} line:{d} column:{d} func:{s}() err:{}\n\r", .{ s.file, s.line, s.column, s.fn_name, err }) catch unreachable);
    };
...
pub fn delete(db: sql3.Database, name: []const u8) bool {
    ...
    if (!existRows(db, defrep.name.string())) return false;
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP, "DELETE FROM defrep  WHERE name='{s}'", .{name}) catch unreachable;
    defer allocREP.free(sqlwrk);

    db.exec(sqlwrk, .{}) catch |err| {
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP, "\n\n\r file:{s} line:{d} column:{d} func:{s}() name:{s} exec:{s} err:{}\n\r", .{ s.file, s.line, s.column, s.fn_name, name, sqlwrk, err }) catch unreachable);
    };
...
// logique LIKE
pub fn lgqLIKE(ldbr: sql3.Database, name: []const u8, like: []const u8) u64 {
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP, "SELECT * FROM defrep WHERE {s} LIKE '%{s}%' ORDER BY name ASC", .{ name, like }) catch unreachable;
    ...
    const select = ldbr.prepare(
        ...
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP, "\n\n\r file:{s} line:{d} column:{d} func:{s}() name:{s}  prepare:{s} err:{}\n\r", .{ s.file, s.line, s.column, s.fn_name, like, sqlwrk, err }) catch unreachable);
    };
    ...
    while (select.step() catch unreachable) |rcd| {
        ...
        rows.append(allocREP, defrep) catch |err| {
            const s = @src();
            @panic(std.fmt.allocPrint(allocREP, "\n\n\r file:{s} line:{d} column:{d} func:{s}() name:{s} select:{s} err:{}\n\r", .{ s.file, s.line, s.column, s.fn_name, like, sqlwrk, err }) catch unreachable);
        };
    }
    return n;
}
...
// logique page down
// n = nbr rows for grid
pub fn pgDown(ldbr: sql3.Database, kpg: []const u8, nrows: u64) u64 {
    var sqlwrk: []const u8 = undefined;
    sqlwrk = std.fmt.allocPrint(allocREP, "SELECT * FROM defrep WHERE name >= '{s}' ORDER BY name ASC", .{kpg}) catch unreachable;
    ...
    const select = ldbr.prepare(
        ...
        const s = @src();
        @panic(std.fmt.allocPrint(allocREP, "\n\n\r file:{s} line:{d} column:{d} func:{s}() name:{s} prepare:{s} err:{}\n\r", .{ s.file, s.line, s.column, s.fn_name, kpg, sqlwrk, err }) catch unreachable);
    };
